    ...
    <section class="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
      <AppCard>
        <div class="border-b border-slate-200 px-5 py-4 dark:border-slate-800">
          ...
          <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">A compact form with basic client-side
            validation.</p>
        </div>
        <form class="space-y-5 p-5" @submit.prevent="submit">
          <div class="grid gap-4 sm:grid-cols-2">
            ...
            <AppInput v-model="form.email" label="Email" type="email" placeholder="alex@company.com"
                      :error="errors.email" required/>
          </div>

          <div class="grid gap-4 sm:grid-cols-2">
            <AppInput v-model="form.company" label="Company" placeholder="Interservice" :error="errors.company"
                      required/>
      ...
      <div class="space-y-4">
        ...
        <AppCard>
          ...
          <pre class="overflow-x-auto p-5 text-xs leading-6 text-slate-600 dark:text-slate-300">{{
              JSON.stringify({
                name: form.name,
                email: form.email,
